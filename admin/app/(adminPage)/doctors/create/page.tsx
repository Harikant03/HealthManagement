...
const CreateDoctor = () => {
  ...
  // 🕒 Optimized Time format (12-hour with AM/PM)
  const formatTimeToAMPM = (time24: string) => {
    if (!time24) return "";
    let [hours, minutes] = time24.split(':');
    let h = parseInt(hours);
    const ampm = h >= 12 ? 'PM' : 'AM';
    h = h % 12 || 12; // 0 becomes 12, 13 becomes 1
  ...
  const handleSlotChange = (index: number, field: string, value: string) => {
    ...
    // UI input update (date ke liye seedha value, time ke liye formatting)
    newSlots[index] = { 
        ...newSlots[index], 
        ...
        <div className="slots-section">
          ...
          {formData.availableSlots.map((slot, index) => (
            <div key={index} className="slot-row">
              ...
              <input 
                type="time" 
                // Note: time input hamesha 24h value expect karta hai, 
                // isliye hum isko display ke liye unformatted hi rakhte hain native picker mein
